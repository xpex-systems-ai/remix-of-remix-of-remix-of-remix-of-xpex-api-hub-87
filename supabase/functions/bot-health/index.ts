...
async function checkAIService(): Promise<ServiceStatus> {
  ...
  try {
    ...
    // Just check if the key exists - don't make actual requests to save costs
    const latency = Date.now() - start;
...
serve(async (req) => {
  console.log('[BOT-HEALTH] Health check requested');
  ...
  try {
    ...
    const services = [database, aiService, runtime];
    ...
    const response: BotHealthResponse = {
      ...
      services,
      endpoints: [
        { path: '/bot/validate', method: 'POST', status: 'available', cost: 1 },
        { path: '/bot/validate-ai', method: 'POST', status: 'available', cost: 1 },
        { path: '/bot/health', method: 'GET', status: 'available', cost: 0 },
      ],
      rate_limits: [
        { tier: 'Free', requests_per_second: 5 },
        { tier: 'Starter', requests_per_second: 10 },
        { tier: 'Scale', requests_per_second: 50 },
        { tier: 'Enterprise', requests_per_second: 500 },
      ],
    };

    console.log('[BOT-HEALTH] Check completed:', overallStatus);
