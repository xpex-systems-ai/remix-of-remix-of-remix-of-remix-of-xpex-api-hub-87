...
async function validateApiKey(apiKey: string): Promise<{ 
  ...
  if (error || !data || data.length === 0) {
    console.log('[AGENT-VALIDATE-AI] API key validation failed');
    return { valid: false };
  }
  ...
  if (!keyData.valid) {
    return { valid: false, isExpired: keyData.is_expired };
  }
...
async function deductCredits(userId: string, amount: number = 1): Promise<{ success: boolean; remainingCredits: number }> {
  ...
  if (error || newCredits < 0) {
    console.error('[AGENT-VALIDATE-AI] Credit deduction error:', error);
  ...
  await supabase.from('credit_transactions').insert({
    ...
    amount: -amount,
    type: 'deduction',
    description: 'Agent API - AI Email validation',
...
serve(async (req) => {
  ...
  try {
    ...
    const ipRateLimit = checkRateLimit(`agent-ai:ip:${clientIp}`, IP_RATE_LIMIT);
    if (!ipRateLimit.allowed) {
      console.log('[AGENT-VALIDATE-AI] IP rate limit exceeded:', clientIp);
    ...
    if (!apiKey) {
      console.log('[AGENT-VALIDATE-AI] Missing API key');
      return new Response(JSON.stringify({
        ok: false,
    ...
    const keyRateLimit = checkRateLimit(`agent-ai:key:${apiKey}`, rateTier);
    if (!keyRateLimit.allowed) {
      console.log('[AGENT-VALIDATE-AI] API key rate limit exceeded for tier:', tier);
      return new Response(JSON.stringify({
        ok: false,
    ...
    if (!email || typeof email !== 'string') {
      ...
      await logUsage(keyValidation.userId!, keyValidation.keyId!, '/agent/validate-ai', 400, responseTime);
    ...
    console.log('[AGENT-VALIDATE-AI] Processing:', email, 'Tier:', tier, 'Sandbox:', isTestMode);

    // 7. Basic validation + MX check
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are an expert email validation AI for the XPEX Agent API. Analyze emails for:
...
Respond ONLY with valid JSON:
{
  ...
  "confidence": 0-100
          ...
          {
            role: 'user',
            content: `Analyze for Agent API validation: ${email}\nDomain: ${domain}\nFormat valid: ${formatValid}\nKnown disposable: ${isDisposable}\nMX Records found: ${mxResult.valid}`
          }
        ],
      }),
    });
    ...
    if (!response.ok) {
      ...
      console.error('[AGENT-VALIDATE-AI] AI Gateway error:', response.status, errorText);
      
      const responseTime = Date.now() - startTime;
      await logUsage(keyValidation.userId!, keyValidation.keyId!, '/agent/validate-ai', response.status, responseTime);
    ...
    try {
      ...
      console.error('[AGENT-VALIDATE-AI] Failed to parse AI response');
    }

    // 10. Deduct credits (skip in sandbox mode)
    ...
    // 11. Log usage
    await logUsage(keyValidation.userId!, keyValidation.keyId!, '/agent/validate-ai', 200, responseTime);
    ...
    const result = {
      ...
      data: {
        email,
        valid: formatValid && !isDisposable && mxResult.valid && (aiAnalysis?.risk_level !== 'high'),
        score: aiAnalysis?.deliverability_score ?? (formatValid && !isDisposable && mxResult.valid ? 85 : 20),
        checks: {
        ...
        domain,
        risk_level: aiAnalysis?.risk_level || (isDisposable ? 'high' : formatValid && mxResult.valid ? 'low' : 'medium'),
        risk_score: aiAnalysis?.risk_score ?? (isDisposable ? 90 : formatValid && mxResult.valid ? 15 : 60),
      ...
      meta: {
        ...
        api_version: '1.0.0',
        endpoint: '/agent/validate-ai'
      }
    };

    console.log('[AGENT-VALIDATE-AI] Success:', email, 'Risk:', result.data.risk_level, 'Time:', responseTime, 'ms');
    ...
    console.error('[AGENT-VALIDATE-AI] Error:', error);
    return new Response(JSON.stringify({
      ok: false,
